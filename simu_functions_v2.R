...
generate_Y_nonconfound<-function(Risk,D,model,p0,p1){
      ...
      for(i in 1:length(beta0)){
        new_D<-D
        if(model!='step'){
          new_D<-D+node("PY", distr = "rconst", const = fun(eval(beta0[i])-5*S+W1/2+W2/2-S^2*(W1>0)-S^2*(W2>0)))+
      ...
      if(model!='step'){
        D<-D+node("PY", distr = "rconst", const = fun(eval(select)-5*S+W1/2+W2/2-S^2*(W1>0)-S^2*(W2>0)))+
          node("Y",distr='rbern',prob=PY)
        }
      else{
